  ...
  return(
    <>
      ...
      <Transition.Root appear show={isDelOpen} as={Fragment}>
        <Dialog as="div" className="relative z-50" onClose={async () => { setDelOpen(false) }}>
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0"
          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              <Transition.Child
                ...
                leaveTo="opacity-0 scale-95"
              ><div>
                <DeleteDialog close={() => { setDelOpen(false); fetchPets() }} userId={clientId} petId = {selectedId}/>
              </div>
              </Transition.Child>
            </div>
          </div>
      ...
      <Transition.Root appear show={isEditOpen} as={Fragment}>
        <Dialog as="div" className="relative z-50" onClose={async () => { setEditOpen(false) }}>
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0"
          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              <Transition.Child
                ...
                leaveTo="opacity-0 scale-95"
              ><div>
                <AddingDialog close={() => { setEditOpen(false); fetchPets() }} id={clientId}/>
              </div>
              </Transition.Child>
            </div>
          </div>
